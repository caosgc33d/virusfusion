...
#VirusFusion_V2.1.pl
#updated date: Nov 18 2014
#V2.1: the version which can deal with UNC bams and normal bams
#V2.2. use new virus database
...
#my $bwa_ref = "/gscmnt/gc3027/dinglab/medseq/fasta/GRCh37PlusSelectedVirus2015.02/GRCh37Plus26Virus.fa";
...
my $anno_bed_b37="/gscuser/scao/gc3027/cnv/brca/all_CDS_and_ncRNA_24Chroms_Contigs_1BasedStart_2bpFlanks_MergedExons";
...
sub bsub_anno{
	...
	print ANNO "perl -e \'\$f_g=\"$ANNO_BED\"; \$f_r=\"$anno_bed_b37\"; \$sn=\$f_g; \$sn=\"$sample_name\"; foreach \$a (\`cat \$f_r\`) { \@ss=split(\"\\t\",\$a); \$chr=\$ss[0]; \$l=\$ss[1]; \$r=\$ss[2]; \$g=\$ss[3]; chomp(\$g); \$gbed{\$g}{\$chr}{\$l}{\$r}=1; } foreach \$a (\`cat \$f_g\`) { \@ss=split(\" \",\$a); \$chr=\$ss[0]; \$pos=\$ss[1];  foreach \$g (keys \%gbed) { foreach \$c (keys \%{\$gbed{\$g}}) { if(\$chr==\$c) { foreach \$l (keys \%{\$gbed{\$g}{\$c}}) { foreach \$r (keys \%{\$gbed{\$g}{\$c}{\$l}}) {  if(\$r>=\$pos && \$l<=\$pos) { \$h{\$g}++; }}}}}}} foreach \$l (\`cat $all_sample_list\`) { \@ss=split(\"\\t\",\$l); \$s_2_t{\$ss[0]}=\$ss[2]; \$s_2_nt{\$ss[0]}=\$ss[1]; } foreach \$g (sort { \$h{\$b} <=> \$h{\$a} } keys %h) { print \$sn,\"\\t\",\$s_2_nt{\$sn},\"\\t\",\$s_2_t{\$sn},\"\\t\",\$g,\"\\t\",\$h{\$g},\"\\n\";}\'> $ANNO_SUMMARY","\n"; 
    print ANNO "fi\n";
    close ANNO;
    $bsub_com = "bsub < $job_files_dir/$current_job_file\n";
    system ( $bsub_com );
